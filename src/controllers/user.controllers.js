...
exports.updateUser = async (req, res) => {
	try {
		...
		if (fullName && phone && gender && dob && address) {
			...
			if (!fullName) {
				return res.status(400).json(errorResponse(1, 'FAILED', 'User `fullName` field is required'))
			}
			// check if phone is empty
			if (!phone) {
				return res.status(400).json(errorResponse(1, 'FAILED', 'User `phone` field is required'))
			}
			// check if gender is empty
			if (!gender) {
				return res.status(400).json(errorResponse(1, 'FAILED', 'User `gender` field is required'))
			}
			// check if address is empty
			if (!address) {
				return res.status(400).json(errorResponse(1, 'FAILED', 'User `address` field is required'))
			}
		}
	} catch (error) {
		res.status(500).json(errorResponse(2, 'SERVER SIDE ERROR', error))
...
exports.deleteUser = async (req, res) => {
	try {
		...
		res.status(200).json(successResponse(0, 'SUCCESS', 'User delete form database successful'))
	} catch (error) {
		res.status(500).json(errorResponse(2, 'SERVER SIDE ERROR', error))
	}
}
...
exports.getUsersList = async (req, res) => {
	try {
		...
		if (!users) {
			return res
				.status(404)
				.json(errorResponse(4, 'UNKNOWN ACCESS', 'Sorry! Any user does not found'))
		}
...
exports.blockedUser = async (req, res) => {
	try {
		...
		if (req?.user?._id?.toString() === req.params.id) {
			return res
				.status(400)
				.json(errorResponse(1, 'FAILED', 'Sorry! You can´t possible self blocked'))
		}
		...
		if (user.status === 'blocked') {
			return res.status(400).json(errorResponse(1, 'FAILED', 'Ops! User already blocked'))
		}
...
exports.unblockedUser = async (req, res) => {
	try {
		...
		if (req?.user?._id?.toString() === req.params.id) {
			return res
				.status(400)
				.json(errorResponse(1, 'FAILED', 'Ops! You can´t possible self unblocked'))
		}
		...
		if (user.status !== 'blocked') {
			return res.status(400).json(errorResponse(1, 'FAILED', 'Sorry! User already unblocked'))
		}
