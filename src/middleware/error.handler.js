...
exports.notFoundRoute = (_req, res, _next) => {
	res.status(404).json(errorResponse(4, 'UNKNOWN ACCESS', 'Sorry! Your request url was not found.'))
}
...
exports.errorHandler = (err, _req, res, next) => {
	...
	if (err.message) {
		...
		res
			.status(500)
			.json(errorResponse(2, 'SERVER SIDE ERROR', 'Something went wrong. There was an error.'))
	}
}
