...
exports.roomReviewAdd = async (req, res) => {
	try {
		...
		if (!rating) {
			return res.status(400).json(errorResponse(1, 'FAILED', '`rating` filed is required'))
		}

		// check `message` filed exits
		if (!message) {
			return res.status(400).json(errorResponse(1, 'FAILED', '`message` filed is required'))
		}
		...
		if (/^[0-9a-fA-F]{24}$/.test(req.params.id)) {
			...
			return res
				.status(400)
				.json(
					errorResponse(1, 'FAILED', 'Something went wrong. Probably booking id missing/incorrect')
				)
		}

		// check room available
		if (!myBooking) {
			return res.status(404).json(errorResponse(4, 'UNKNOWN ACCESS', 'Booking does not exist'))
		}
		...
		if (myBooking.reviews) {
			return res
				.status(400)
				.json(errorResponse(1, 'FAILED', 'Sorry! This booking already add an review'))
		}
		...
		if (myBooking.booking_status !== 'in-reviews') {
			return res
				.status(400)
				.json(errorResponse(1, 'FAILED', 'Invalid booking status for adding a review'))
		}
		...
		res
			.status(201)
			.json(successResponse(0, 'SUCCESS', 'Your room booking order placed successful', savedReview))
	} catch (error) {
...
exports.getRoomReviewsList = async (req, res) => {
	try {
		...
		if (/^[0-9a-fA-F]{24}$/.test(req.params.room_id)) {
			myReviews = await Review.find({ room_id: req.params.room_id }).populate('user_id')
		} else {
			return res
				.status(400)
				.json(
		...
		if (!myReviews) {
			return res.status(404).json(errorResponse(4, 'UNKNOWN ACCESS', 'Review does not exist'))
		}
		...
		res.status(200).json(
			successResponse(0, 'SUCCESS', 'Reviews list retrieved successful', {
				...
				total_page: req?.query?.keyword
					? Math.ceil(findReviews.length / req.query.limit)
					: Math.ceil(myReviews.length / req.query.limit),
...
exports.editSelfRoomReview = async (req, res) => {
	try {
		...
		if (/^[0-9a-fA-F]{24}$/.test(req.params.review_id)) {
			myReviews = await Review.findById(req.params.review_id).populate('user_id')
		} else {
			return res
				.status(400)
				.json(
		...
		if (myReviews?.user_id?.id !== req?.user?.id) {
			return res
				.status(406)
				.json(errorResponse(6, 'UNABLE TO ACCESS', 'Sorry! You can update only self room reviews'))
		}
		...
		res
			.status(200)
			.json(successResponse(0, 'SUCCESS', 'Your room review update successful', updatedReview))
	} catch (error) {
		res.status(500).json(errorResponse(2, 'SERVER SIDE ERROR', error))
	}
}
