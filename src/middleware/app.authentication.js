...
exports.isAuthenticatedUser = async (req, res, next) => {
	try {
		...
		if (!authorization) {
			return res
				.status(403)
				.json(errorResponse(3, 'ACCESS FORBIDDEN', 'Authorization headers is required'))
		}
		...
		jwt.verify(token, process.env.JWT_SECRET_KEY, async (err, dec) => {
			...
			if (!user) {
				return res
					.status(404)
					.json(errorResponse(4, 'UNKNOWN ACCESS', 'Authorization headers is missing/invalid'))
			}
			...
			if (user.status === 'login') {
				...
				return res
					.status(401)
					.json(errorResponse(1, 'FAILED', 'Unauthorized access. Please login to continue'))
			}
		})
	} catch (error) {
...
exports.isAdmin = async (req, res, next) => {
	try {
		...
		if (!user) {
			return res.status(404).json(errorResponse(4, 'UNKNOWN ACCESS', 'Sorry, User does not exist'))
		}
