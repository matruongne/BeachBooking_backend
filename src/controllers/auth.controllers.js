	...
	try {
		...
		if (userName && fullName && email && password && dob && address) {
			...
			if (findUserName) {
				...
				return res
					.status(409)
					.json(errorResponse(9, 'ALREADY EXIST', 'Sorry, Username already exists'))
			}
			...
			if (findEmail) {
				...
				return res
					.status(409)
					.json(errorResponse(9, 'ALREADY EXIST', 'Sorry, Email already exists'))
			}
			...
			if (findPhone) {
				...
				return res
					.status(409)
					.json(errorResponse(9, 'ALREADY EXIST', 'Sorry, Phone number already exists'))
			}
...
exports.logoutUser = async (req, res) => {
	try {
		...
		if (!user) {
			return res
				.status(404)
				.json(errorResponse(4, 'UNKNOWN ACCESS', 'Unauthorized access. Please login to continue'))
		}
		...
		// response user
		res.status(200).json(successResponse(0, 'SUCCESS', 'User logged out successful'))
	} catch (error) {
		res.status(500).json(errorResponse(2, 'SERVER SIDE ERROR', error))
	}
}
	...
	try {
		if (req.params.token && req.body.password && req.body.confirmPassword) {
			...
			if (req.body.password !== req.body.confirmPassword) {
				return res
					.status(400)
					.json(errorResponse(1, 'FAILED', 'Password and Confirm password does not match'))
			}
			...
			res.status(200).json(successResponse(0, 'SUCCESS', 'User password reset successful'))
		} else {
			return res.status(400).json(errorResponse(1, 'FAILED', 'Please enter all required fields'))
		}
	} catch (error) {
...
exports.sendEmailVerificationLink = async (req, res) => {
	try {
		...
		if (user.verified) {
			return res.status(400).json(errorResponse(1, 'FAILED', 'Ops! Your mail already verified'))
		}
