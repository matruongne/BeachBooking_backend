	...
	try {
		...
		if (/^[0-9a-fA-F]{24}$/.test(req.params.id)) {
			...
			return res
				.status(400)
				.json(
					errorResponse(1, 'FAILED', 'Something went wrong. Probably room id missing/incorrect')
				)
		}
		...
		if (myRoom.room_status === 'unavailable') {
			return res
				.status(400)
				.json(errorResponse(1, 'FAILED', "Sorry! Current your sleeted room can't available"))
		}
...
exports.getBookingOrderByUserId = async (req, res) => {
	try {
		...
		if (!myBooking || myBooking.length === 0) {
			return res
				.status(404)
				.json(errorResponse(4, 'UNKNOWN ACCESS', 'No bookings found for the specified user'))
		}

		// filtering booking orders based on different types query
		const bookingQuery = new MyQueryHelper(
			Booking.find({ booking_by: req.user.id })
				.populate('room_id')
				.populate('booking_by')
				.populate({ path: 'reviews', populate: { path: 'user_id', model: 'Users' } }),
			req.query
		)
			.sort()
			.paginate()
		...
		const mapperBooking = findBooking?.map(data => ({
			...
			room: {
				...
				room_images: data?.room_id?.room_images?.map(img => ({
		...
		res.status(200).json(
			successResponse(0, 'SUCCESS', 'Booking list retrieved successful', {
				rows: mapperBooking,
				total_rows: myBooking.length,
				response_rows: findBooking.length,
				total_page: req?.query?.keyword
					? Math.ceil(findBooking.length / req.query.limit)
					: Math.ceil(myBooking.length / req.query.limit),
...
exports.cancelSelfBookingOrder = async (req, res) => {
	try {
		...
		res
			.status(200)
			.json(successResponse(0, 'SUCCESS', 'Booking order has been canceled successful', booking))
	} catch (error) {
		res.status(500).json(errorResponse(2, 'SERVER SIDE ERROR', error))
	}
}
...
exports.updatedBookingOrderByAdmin = async (req, res) => {
	try {
		...
		if (!req.body.booking_status) {
			return res.status(400).json(errorResponse(1, 'FAILED', '`booking_status` filed is required'))
		}
