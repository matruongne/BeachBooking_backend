...
exports.createRoom = async (req, res) => {
	try {
		...
		if (!room_description) {
			...
			return res
				.status(400)
				.json(errorResponse(1, 'FAILED', '`room_description` filed is required'))
		}
		...
		if (!extra_facilities[0]) {
			...
			return res
				.status(400)
				.json(errorResponse(1, 'FAILED', 'Minimum 1 `extra_facilities` filed is required'))
		}
		...
		if (!req.files[0]) {
			...
			return res
				.status(400)
				.json(errorResponse(1, 'FAILED', 'Minimum 1 `room_images` filed is required '))
		}
		...
		if (roomName) {
			...
			return res
				.status(409)
				.json(errorResponse(9, 'ALREADY EXIST', 'Sorry, `room_name` already exists'))
		}
		...
		if (roomSlug) {
			...
			return res
				.status(409)
				.json(errorResponse(9, 'ALREADY EXIST', 'Sorry, `room_slug` already exists'))
		}
...
exports.getRoomsList = async (req, res) => {
	try {
		...
		res.status(200).json(
			successResponse(0, 'SUCCESS', 'Rooms list data found successful', {
				rows: mappedRooms,
...
exports.getRoomByIdOrSlugName = async (req, res) => {
	try {
		...
		res
			.status(200)
			.json(successResponse(0, 'SUCCESS', 'User information get successful', organizedRoom))
	} catch (error) {
		res.status(500).json(errorResponse(2, 'SERVER SIDE ERROR', error))
	}
}
...
exports.getFeaturedRoomsList = async (req, res) => {
	try {
		...
		res.status(200).json(
			successResponse(0, 'SUCCESS', 'Featured rooms list data found successful', {
				rows: mappedRooms,
				total_rows: rooms.length,
				response_rows: findRooms.length,
				total_page: req?.query?.keyword
